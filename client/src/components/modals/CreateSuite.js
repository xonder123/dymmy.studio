...
import {createSuite} from "../../http/deviceAPI"

const CreateSuite = observer(({show, onHide}) => {
  const {suite} = useContext(Context)
  ...
  const removeInfo = (number) => {
    ...
    const addSuite =() => {
      ...
      formData.append('barandId', suite.selectedBrand.id)
      formData.append('typeId', suite.selectedType.id)
      formData.append('info', JSON.stringify(info))

      createSuite(formData).then(data => onHide())
    }
 ...
 const selectFilew = e => {
  ...
  useEffect(() => {
    fetchTypes().then(data => suite.setTypes(data))
    fetchBrand().then(data => suite.setBrands(data))
    fetchDeviceslides().then(data => suite.setDeviceslides(data))
  ...
  return (
      ...
      <Modal.Footer>
        ...
        <Button variant="outline-success" onClick={addSuite}>добавить</Button>
